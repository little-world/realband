...
$$.Closure$onStudentsLoaded = {"": "Closure;call$1,$name"};
...
["zoek.dart", "zoek.dart", , {
main: function() {
  $.HttpRequest_getString("/students", null, null).then$1($.onStudentsLoaded$closure);
},

onStudentsLoaded: function(responseText) {
  var json, t1, $length, t2, truncated, cl, cols, i, end;
  json = $.parse(responseText, null);
  t1 = $.getInterceptor$asx(json);
  $length = t1.get$length(json);
  if (typeof $length !== "number")
    throw $length.$div();
  t2 = Math.ceil($length / 4);
  if (isNaN(t2))
    $.throwExpression(new $.UnsupportedError("NaN"));
  if (t2 == Infinity || t2 == -Infinity)
    $.throwExpression(new $.UnsupportedError("Infinity"));
  truncated = t2 < 0 ? Math.ceil(t2) : Math.floor(t2);
  cl = truncated == -0.0 ? 0 : truncated;
  $.Primitives_printString($.JSNumber_methods.toString$0(cl));
  cols = $.List_List($);
  for (i = 0; i < 4; ++i) {
    t2 = i * cl;
    end = t2 + cl;
    ...
    cols.push(t1.sublist$2(json, t2, end));
  }
  $.Primitives_printString($.JSArray_methods.toString$0(cols));
  $.students = cols;
}}],
]);
...
$.onStudentsLoaded$closure = new $.Closure$onStudentsLoaded($.onStudentsLoaded, "onStudentsLoaded$closure");
...
//@ sourceMappingURL=zoek.dart.js.map
